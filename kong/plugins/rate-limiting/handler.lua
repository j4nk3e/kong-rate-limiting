...
local function get_identifier(conf)
    ...
    elseif conf.limit_by == "consumer" then
        identifier =
            (kong.client.get_consumer() or kong.client.get_credential() or EMPTY).id
    ...
    elseif conf.limit_by == "path" then
        ...
        if req_path == conf.path then identifier = req_path end
    end
...
local function get_usage(conf, identifier, current_timestamp, limits)
    ...
    for period, limit in pairs(limits) do
        ...
        -- Recording usage
        usage[period] = {limit = limit, remaining = remaining}

        if remaining <= 0 then stop = period end
    end
    ...
    if usage then
        ...
        if not conf.hide_client_headers then
            ...
            for k, v in pairs(usage) do
                ...
                if not limit or (current_remaining < remaining) or
                    (current_remaining == remaining and current_window > window) then
                    limit = current_limit
                    ...
                    reset = max(1, window -
                                    floor(
                                        (current_timestamp - timestamps[k]) /
                                            1000))
                end
